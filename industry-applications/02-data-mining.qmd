...
subtitle: "User Profiling, POI Linking, and Review Mining"
format:
  html:
    toc: true
    toc-depth: 3
...
scatter1 = axes[0, 0].scatter(
    ...
    user_profile['avg_rating'],  # Use avg_rating, not star_rating
...
axes[0, 0].set_ylim(0.5, 5.5)  # Proper rating range
...
scatter2 = axes[0, 1].scatter(
    ...
    user_profile['rating_std'],  # Use rating_std from user_profile
    c=user_profile['user_segment'],
    cmap='viridis',
    alpha=0.6,
    s=30,
...
axes[1, 1].set_yscale('symlog', linthresh=1)  # Handle zeros gracefully
...
print(f"Total users plotted: {len(user_profile):,}")
print(f"Avg rating range: {user_profile['avg_rating'].min():.2f} to {user_profile['avg_rating'].max():.2f}")
print(f"Review count range: {user_profile['review_count'].min()} to {user_profile['review_count'].max()}")
print(f"Users with helpful votes: {(user_profile['avg_helpful_votes'] > 0).sum():,}")
```
...
if len(importance_dict) == 0:
    print("⚠️ No 'gain' scores found, using 'weight' instead")
...
if importance_df['importance'].sum() > 0:
    ...
    print("\n⚠️ Warning: All importances are zero!")
...
def compute_ranking_metrics(y_true, y_pred_proba, k=10):
    ...
    top_k_idx = np.argsort(y_pred_proba)[-k:]  # ⚠️ This gets HIGHEST scores

    # Precision@K: % of top-K that are actually positive
    precision_k = y_true.iloc[top_k_idx].mean()  # ⚠️ All are positive!
```

**The issue:** With an imbalanced dataset (84.9% positive reviews), the top-K predictions are almost always positive, giving artificially high precision.

## Fix: Proper Ranking Evaluation

Replace section 4.9 with this corrected version:
